...
const _ = "[a-zA-Z]\\w*", y = "[a-zA-Z_]\\w*", w = "\\b\\d+(\\.\\d+)?", x = "(-?)(\\b0[xX][a-fA-F0-9]+|(\\b\\d+(\\.\\d*)?|\\.\\d+)([eE][-+]?\\d+)?)", S = "\\b(0b[01]+)", v = { begin: "\\\\[\\s\\S]", relevance: 0 }, A = { scope: "string", begin: "'", end: "'", illegal: "\\n", contains: [v] }, R = { scope: "string", begin: '"', end: '"', illegal: "\\n", contains: [v] }, O = function(e2, n2, t2 = {}) {
...
}, N = O("//", "$"), k = O("/\\*", "\\*/"), M = O("#", "$"), j = { scope: "number", begin: w, relevance: 0 }, B = { scope: "number", begin: x, relevance: 0 }, T = { scope: "number", begin: S, relevance: 0 }, L = { scope: "regexp", begin: /\/(?=[^/\n]*\/)/, end: /\/[gimuy]*/, contains: [v, { begin: /\[/, end: /\]/, relevance: 0, contains: [v] }] }, I = { scope: "title", begin: _, relevance: 0 }, C = { scope: "title", begin: y, relevance: 0 }, $ = { begin: "\\.\\s*" + y, relevance: 0 };
var H = Object.freeze({ __proto__: null, APOS_STRING_MODE: A, BACKSLASH_ESCAPE: v, BINARY_NUMBER_MODE: T, BINARY_NUMBER_RE: S, COMMENT: O, C_BLOCK_COMMENT_MODE: k, C_LINE_COMMENT_MODE: N, C_NUMBER_MODE: B, C_NUMBER_RE: x, END_SAME_AS_BEGIN: function(e2) {
  return Object.assign(e2, { "on:begin": (e3, n2) => {
    n2.data._beginMatch = e3[1];
  }, "on:end": (e3, n2) => {
    n2.data._beginMatch !== e3[1] && n2.ignoreMatch();
  } });
}, HASH_COMMENT_MODE: M, IDENT_RE: _, MATCH_NOTHING_RE: /\b\B/, METHOD_GUARD: $, NUMBER_MODE: j, NUMBER_RE: w, PHRASAL_WORDS_MODE: { begin: /\b(a|an|the|are|I'm|isn't|don't|doesn't|won't|but|just|should|pretty|simply|enough|gonna|going|wtf|so|such|will|you|your|they|like|more)\b/ }, QUOTE_STRING_MODE: R, REGEXP_MODE: L, RE_STARTERS_RE: "!|!=|!==|%|%=|&|&&|&=|\\*|\\*=|\\+|\\+=|,|-|-=|/=|/|:|;|<<|<<=|<=|<|===|==|=|>>>=|>>=|>=|>>>|>>|>|\\?|\\[|\\{|\\(|\\^|\\^=|\\||\\|=|\\|\\||~", SHEBANG: (e2 = {}) => {
  const n2 = /^#![ ]*\//;
...
}, TITLE_MODE: I, UNDERSCORE_IDENT_RE: y, UNDERSCORE_TITLE_MODE: C });
function P(e2, n2) {
...
function ne(e2, n2, { key: t2 }) {
  ...
  for (let e3 = 1; e3 <= n2.length; e3++) s2[e3 + i2] = o2[e3], r2[e3 + i2] = true, i2 += b(n2[e3 - 1]);
...
function te(e2) {
  !function(e3) {
    ...
    if (Array.isArray(e3.begin)) {
      if (e3.skip || e3.excludeBegin || e3.returnBegin) throw V("skip, excludeBegin, returnBegin not compatible with beginScope: {}"), ee;
      if ("object" != typeof e3.beginScope || null === e3.beginScope) throw V("beginScope must be object"), ee;
      ne(e3, e3.begin, { key: "beginScope" }), e3.begin = E(e3.begin, { joinWith: "" });
    }
  }(e2), function(e3) {
    if (Array.isArray(e3.end)) {
      if (e3.skip || e3.excludeEnd || e3.returnEnd) throw V("skip, excludeEnd, returnEnd not compatible with endScope: {}"), ee;
      if ("object" != typeof e3.endScope || null === e3.endScope) throw V("endScope must be object"), ee;
...
function ie(e2) {
  ...
  return e2.classNameAliases = o(e2.classNameAliases || {}), function t3(r2, s2) {
    const a2 = r2;
    if (r2.isCompiled) return a2;
    [D, z, te, W].forEach((e3) => e3(r2, s2)), e2.compilerExtensions.forEach((e3) => e3(r2, s2)), r2.__beforeBegin = null, [U, F, K].forEach((e3) => e3(r2, s2)), r2.isCompiled = true;
    let c2 = null;
...
const se = i, ae = o, ce = Symbol("nomatch"), le = function(e2) {
  ...
  function m2(e3, n2, t2) {
    ...
    R2("before:highlight", r3);
    const s3 = r3.result ? r3.result : E2(r3.language, r3.code, t2);
    return s3.code = r3.code, R2("after:highlight", s3), s3;
  }
  function E2(e3, n2, o3, r3) {
    const c3 = /* @__PURE__ */ Object.create(null);
    function l2() {
      if (!R3.keywords) return void N2.addText(k2);
      let e4 = 0;
      R3.keywordPatternRe.lastIndex = 0;
      let n3 = R3.keywordPatternRe.exec(k2), t2 = "";
      for (; n3; ) {
        t2 += k2.substring(e4, n3.index);
        const o4 = x3.case_insensitive ? n3[0].toLowerCase() : n3[0], r4 = (i3 = o4, R3.keywords[i3]);
        if (r4) {
          const [e5, i4] = r4;
          if (N2.addText(t2), t2 = "", c3[o4] = (c3[o4] || 0) + 1, c3[o4] <= 7 && (M2 += i4), e5.startsWith("_")) t2 += n3[0];
          else {
            const t3 = x3.classNameAliases[e5] || e5;
            d2(n3[0], t3);
          }
        } else t2 += n3[0];
        e4 = R3.keywordPatternRe.lastIndex, n3 = R3.keywordPatternRe.exec(k2);
      }
      var i3;
      t2 += k2.substring(e4), N2.addText(t2);
    }
    function u2() {
      null != R3.subLanguage ? function() {
        if ("" === k2) return;
        let e4 = null;
        if ("string" == typeof R3.subLanguage) {
          if (!i2[R3.subLanguage]) return void N2.addText(k2);
          e4 = E2(R3.subLanguage, k2, true, O2[R3.subLanguage]), O2[R3.subLanguage] = e4._top;
        } else e4 = _2(k2, R3.subLanguage.length ? R3.subLanguage : null);
        R3.relevance > 0 && (M2 += e4.relevance), N2.__addSublanguage(e4._emitter, e4.language);
      }() : l2(), k2 = "";
    }
    function d2(e4, n3) {
      "" !== e4 && (N2.startScope(n3), N2.addText(e4), N2.endScope());
    }
    function h2(e4, n3) {
      let t2 = 1;
      ...
      for (; t2 <= i3; ) {
        ...
        const i4 = x3.classNameAliases[e4[t2]] || e4[t2], o4 = n3[t2];
        i4 ? d2(o4, i4) : (k2 = o4, l2(), k2 = ""), t2++;
      }
    }
    function f2(e4, n3) {
      return e4.scope && "string" == typeof e4.scope && N2.openNode(x3.classNameAliases[e4.scope] || e4.scope), e4.beginScope && (e4.beginScope._wrap ? (d2(k2, x3.classNameAliases[e4.beginScope._wrap] || e4.beginScope._wrap), k2 = "") : e4.beginScope._multi && (h2(e4.beginScope, n3), k2 = "")), R3 = Object.create(e4, { parent: { value: R3 } }), R3;
    }
    ...
    function b3(e4) {
      return 0 === R3.matcher.regexIndex ? (k2 += e4[0], 1) : (T2 = true, 0);
    }
    function m3(e4) {
      const t2 = e4[0], i3 = n2.substring(e4.index), o4 = p2(R3, e4, i3);
      if (!o4) return ce;
      const r4 = R3;
      R3.endScope && R3.endScope._wrap ? (u2(), d2(t2, R3.endScope._wrap)) : R3.endScope && R3.endScope._multi ? (u2(), h2(R3.endScope, e4)) : r4.skip ? k2 += t2 : (r4.returnEnd || r4.excludeEnd || (k2 += t2), u2(), r4.excludeEnd && (k2 = t2));
      do {
        R3.scope && N2.closeNode(), R3.skip || R3.subLanguage || (M2 += R3.relevance), R3 = R3.parent;
      } while (R3 !== o4.parent);
    ...
    function w3(i3, r4) {
      const a3 = r4 && r4[0];
      if (k2 += i3, null == a3) return u2(), 0;
      if ("begin" === y3.type && "end" === r4.type && y3.index === r4.index && "" === a3) {
        if (k2 += n2.slice(r4.index, r4.index + 1), !s2) {
          const n3 = new Error(`0 width match regex (${e3})`);
          throw n3.languageName = e3, n3.badRule = y3.rule, n3;
        }
        return 1;
      }
      if (y3 = r4, "begin" === r4.type) return function(e4) {
        const n3 = e4[0], i4 = e4.rule, o4 = new t(i4), r5 = [i4.__beforeBegin, i4["on:begin"]];
        for (const t2 of r5) if (t2 && (t2(e4, o4), o4.isMatchIgnored)) return b3(n3);
        return i4.skip ? k2 += n3 : (i4.excludeBegin && (k2 += n3), u2(), i4.returnBegin || i4.excludeBegin || (k2 = n3)), f2(i4, e4), i4.returnBegin ? 0 : n3.length;
      }(r4);
      if ("illegal" === r4.type && !o3) {
        const e4 = new Error('Illegal lexeme "' + a3 + '" for mode "' + (R3.scope || "<unnamed>") + '"');
        throw e4.mode = R3, e4;
      }
      if ("end" === r4.type) {
        const e4 = m3(r4);
      ...
      if ("illegal" === r4.type && "" === a3) return 1;
      if (B2 > 1e5 && B2 > 3 * r4.index) {
        throw new Error("potential infinite loop, way more iterations than matches");
      }
      return k2 += a3, a3.length;
    }
    const x3 = S2(e3);
    if (!x3) throw V(a2.replace("{}", e3)), new Error('Unknown language: "' + e3 + '"');
    const v3 = ie(x3);
    let A3 = "", R3 = r3 || v3;
    const O2 = {}, N2 = new g2.__emitter(g2);
    !function() {
      const e4 = [];
      for (let n3 = R3; n3 !== x3; n3 = n3.parent) n3.scope && e4.unshift(n3.scope);
      e4.forEach((e5) => N2.openNode(e5));
    }();
    let k2 = "", M2 = 0, j2 = 0, B2 = 0, T2 = false;
    try {
      if (x3.__emitTokens) x3.__emitTokens(n2, N2);
      else {
        for (R3.matcher.considerAll(); ; ) {
          B2++, T2 ? T2 = false : R3.matcher.considerAll(), R3.matcher.lastIndex = j2;
          const e4 = R3.matcher.exec(n2);
          if (!e4) break;
          const t2 = w3(n2.substring(j2, e4.index), e4);
          j2 = e4.index + t2;
        }
        w3(n2.substring(j2));
      }
      return N2.finalize(), A3 = N2.toHTML(), { language: e3, value: A3, relevance: M2, illegal: false, _emitter: N2, _top: R3 };
    } catch (t2) {
      if (t2.message && t2.message.includes("Illegal")) return { language: e3, value: se(n2), illegal: true, relevance: 0, _illegalBy: { message: t2.message, index: j2, context: n2.slice(j2 - 100, j2 + 100), mode: t2.mode, resultSoFar: A3 }, _emitter: N2 };
      if (s2) return { language: e3, value: se(n2), illegal: false, relevance: 0, errorRaised: t2, _emitter: N2, _top: R3 };
      throw t2;
    }
  }
  function _2(e3, n2) {
    ...
    const t2 = function(e4) {
      const n3 = { value: se(e4), illegal: false, relevance: 0, _top: c2, _emitter: new g2.__emitter(g2) };
      return n3._emitter.addText(e4), n3;
    }(e3), o3 = n2.filter(S2).filter(A2).map((n3) => E2(n3, e3, false));
    o3.unshift(t2);
    const r3 = o3.sort((e4, n3) => {
      if (e4.relevance !== n3.relevance) return n3.relevance - e4.relevance;
      if (e4.language && n3.language) {
        if (S2(e4.language).supersetOf === n3.language) return 1;
        if (S2(n3.language).supersetOf === e4.language) return -1;
      }
      return 0;
    }), [s3, a3] = r3, l2 = s3;
  ...
  function y2(e3) {
    ...
    const t2 = function(e4) {
      ...
      return n3.split(/\s+/).find((e5) => b2(e5) || S2(e5));
    }(e3);
    if (b2(t2)) return;
    if (R2("before:highlightElement", { el: e3, language: t2 }), e3.dataset.highlighted) return void console.log("Element previously highlighted. To highlight again, first unset `dataset.highlighted`.", e3);
    ...
    n2 = e3;
    const i3 = n2.textContent, r3 = t2 ? m2(i3, { language: t2, ignoreIllegals: true }) : _2(i3);
    ...
    }(e3, t2, r3.language), e3.result = { language: r3.language, re: r3.relevance, relevance: r3.relevance }, r3.secondBest && (e3.secondBest = { language: r3.secondBest.language, relevance: r3.secondBest.relevance }), R2("after:highlightElement", { el: e3, result: r3, text: i3 });
  }
  let w2 = false;
  function x2() {
    if ("loading" === document.readyState) return void (w2 = true);
  ...
  "undefined" != typeof window && window.addEventListener && window.addEventListener("DOMContentLoaded", function() {
    w2 && x2();
  }, false), Object.assign(e2, { highlight: m2, highlightAuto: _2, highlightAll: x2, highlightElement: y2, highlightBlock: function(e3) {
    return Y("10.7.0", "highlightBlock will be removed entirely in v12.0"), Y("10.7.0", "Please use highlightElement now."), y2(e3);
    ...
    try {
      ...
      if (V("Language definition for '{}' could not be registered.".replace("{}", n2)), !s2) throw e3;
      V(e3), o3 = c2;
    }
  ...
  }, getLanguage: S2, registerAliases: v2, autoDetection: A2, inherit: ae, addPlugin: function(e3) {
    !function(e4) {
...
function he(e2 = document) {
  e2.querySelectorAll('code[lang="python"], code[lang="py3"]').forEach((e3) => {
    de.highlightElement(e3);
  });
}
de.registerLanguage("python", (e2) => function(e3) {
  const n2 = e3.regex, t2 = /[\p{XID_Start}_]\p{XID_Continue}*/u, i2 = ["and", "as", "assert", "async", "await", "break", "case", "class", "continue", "def", "del", "elif", "else", "except", "finally", "for", "from", "global", "if", "import", "in", "is", "lambda", "match", "nonlocal|10", "not", "or", "pass", "raise", "return", "try", "while", "with", "yield"], o2 = { $pattern: /[A-Za-z]\w+|__\w+__/, keyword: i2, built_in: ["__import__", "abs", "all", "any", "ascii", "bin", "bool", "breakpoint", "bytearray", "bytes", "callable", "chr", "classmethod", "compile", "complex", "delattr", "dict", "dir", "divmod", "enumerate", "eval", "exec", "filter", "float", "format", "frozenset", "getattr", "globals", "hasattr", "hash", "help", "hex", "id", "input", "int", "isinstance", "issubclass", "iter", "len", "list", "locals", "map", "max", "memoryview", "min", "next", "object", "oct", "open", "ord", "pow", "print", "property", "range", "repr", "reversed", "round", "set", "setattr", "slice", "sorted", "staticmethod", "str", "sum", "super", "tuple", "type", "vars", "zip"], literal: ["__debug__", "Ellipsis", "False", "None", "NotImplemented", "True"], type: ["Any", "Callable", "Coroutine", "Dict", "List", "Literal", "Generic", "Optional", "Sequence", "Set", "Tuple", "Type", "Union"] }, r2 = { className: "meta", begin: /^(>>>|\.\.\.) / }, s2 = { className: "subst", begin: /\{/, end: /\}/, keywords: o2, illegal: /#/ }, a2 = { begin: /\{\{/, relevance: 0 }, c2 = { className: "string", contains: [e3.BACKSLASH_ESCAPE], variants: [{ begin: /([uU]|[bB]|[rR]|[bB][rR]|[rR][bB])?'''/, end: /'''/, contains: [e3.BACKSLASH_ESCAPE, r2], relevance: 10 }, { begin: /([uU]|[bB]|[rR]|[bB][rR]|[rR][bB])?"""/, end: /"""/, contains: [e3.BACKSLASH_ESCAPE, r2], relevance: 10 }, { begin: /([fF][rR]|[rR][fF]|[fF])'''/, end: /'''/, contains: [e3.BACKSLASH_ESCAPE, r2, a2, s2] }, { begin: /([fF][rR]|[rR][fF]|[fF])"""/, end: /"""/, contains: [e3.BACKSLASH_ESCAPE, r2, a2, s2] }, { begin: /([uU]|[rR])'/, end: /'/, relevance: 10 }, { begin: /([uU]|[rR])"/, end: /"/, relevance: 10 }, { begin: /([bB]|[bB][rR]|[rR][bB])'/, end: /'/ }, { begin: /([bB]|[bB][rR]|[rR][bB])"/, end: /"/ }, { begin: /([fF][rR]|[rR][fF]|[fF])'/, end: /'/, contains: [e3.BACKSLASH_ESCAPE, a2, s2] }, { begin: /([fF][rR]|[rR][fF]|[fF])"/, end: /"/, contains: [e3.BACKSLASH_ESCAPE, a2, s2] }, e3.APOS_STRING_MODE, e3.QUOTE_STRING_MODE] }, l2 = "[0-9](_?[0-9])*", g2 = `(\\b(${l2}))?\\.(${l2})|\\b(${l2})\\.`, u2 = `\\b|${i2.join("|")}`, d2 = { className: "number", relevance: 0, variants: [{ begin: `(\\b(${l2})|(${g2}))[eE][+-]?(${l2})[jJ]?(?=${u2})` }, { begin: `(${g2})[jJ]?` }, { begin: `\\b([1-9](_?[0-9])*|0+(_?0)*)[lLjJ]?(?=${u2})` }, { begin: `\\b0[bB](_?[01])+[lL]?(?=${u2})` }, { begin: `\\b0[oO](_?[0-7])+[lL]?(?=${u2})` }, { begin: `\\b0[xX](_?[0-9a-fA-F])+[lL]?(?=${u2})` }, { begin: `\\b(${l2})[jJ](?=${u2})` }] }, h2 = { className: "comment", begin: n2.lookahead(/# type:/), end: /$/, keywords: o2, contains: [{ begin: /# type:/ }, { begin: /#/, end: /\b\B/, endsWithParent: true }] }, f2 = { className: "params", variants: [{ className: "", begin: /\(\s*\)/, skip: true }, { begin: /\(/, end: /\)/, excludeBegin: true, excludeEnd: true, keywords: o2, contains: ["self", r2, d2, c2, e3.HASH_COMMENT_MODE] }] };
  return s2.contains = [c2, d2, r2], { name: "Python", aliases: ["py", "gyp", "ipython"], unicodeRegex: true, keywords: o2, illegal: /(<\/|\?)|=>/, contains: [r2, d2, { begin: /\bself\b/ }, { beginKeywords: "if", relevance: 0 }, c2, h2, e3.HASH_COMMENT_MODE, { match: [/\bdef/, /\s+/, t2], scope: { 1: "keyword", 3: "title.function" }, contains: [f2] }, { variants: [{ match: [/\bclass/, /\s+/, t2, /\s*/, /\(\s*/, t2, /\s*\)/] }, { match: [/\bclass/, /\s+/, t2] }], scope: { 1: "keyword", 3: "title.class", 6: "title.class.inherited" } }, { className: "meta", begin: /^[\t ]*@/, end: /(?=#)|$/, contains: [d2, f2, c2] }] };
